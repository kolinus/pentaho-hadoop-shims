...
public class NamedConfigurationLoader {
  ...
  public static void load( Class<?> clazz ) {
    NamedConfigurationManager ncm = NamedConfigurationManager.getInstance();
...
/*    
    try {
      Document doc = XMLHandler.loadXMLFile( xmlIn );
      Node n = XMLHandler.getSubNode( doc, NamedConfiguration.XML_TAG );
      configuration = new NamedConfiguration( n );
      configuration.setSubType( clazz.getName() );
    } catch ( Exception e ) {
      e.printStackTrace();
      configuration.setName( HasNamedConfiguration.INVALID_CONFIG );
